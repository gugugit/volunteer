    ...
    */
    static function instance($pConfig = 'default'){
        if(empty(self::$instance[$pConfig])){
            $config = Config::db('mysql',$pConfig);
            self::$instance[$pConfig] = @new PDO($config['dsn'],$config['username'],$config['password'],array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
        }
    ...
    private function _filter(&$datas){
        ...
        foreach($datas as $k1 => &$v1){
            isset($tFields[$k1]) || Msg::ajax('系统错误[DBF]',$k1);
            $v1 = strtr($v1, array('\\' => '', "'" => "\'"));
        }
     ...
     */
    function exec($sql){
        if (defined('DEBUG')) \Helper\File::write(APP_PATH . '/log/sql.log', $sql);
     ...
     * @param string $opt 键值操作
     * @param string $keys 键值
     *
     * @return array
     */
    function getFields($table = '', $opt = '', $keys = ''){
        static $fields = array();
        $table || $table = $this->tablename;
        # 静态 读取表字段
        if(empty($fields[$table])){
            # 缓存 读取表字段
            if(is_file($tFile = APP_PATH.'/cache/db/fields/'.$table.'.json')){
                $fields[$table] = json_decode(file_get_contents($tFile), true);
            }
            # 数据库 读取表字段
            else {
                $fields[$table] = array();
                ...
                if($tQuery = $this->db->query("SHOW FULL FIELDS FROM `$table`")){
                    foreach($tQuery->fetchAll(2) as $v1){
                        $fields[$table][$v1['Field']] = array('type' => $v1['Type'], 'default' => $v1['Default'], 'comment' => $v1['Comment']);
                    }
                    file_put_contents($tFile, json_encode($fields[$table], JSON_UNESCAPED_UNICODE));
                    chmod($tFile, 0777);
                }
            }
        }
        ...
        if($opt){
            $data = array();
            if($keys) $keys = array_flip(explode(',', $keys));
            # 删除指定KEY
            if('del' == $opt){
                foreach ($fields[$table] as $k1 => $v1) if(!isset($keys[$k1])) $data[$k1] = $v1;
            }
            # 保留指定KEY
            elseif('save' == $opt){
                foreach ($fields[$table] as $k1 => $v1) if(isset($keys[$k1])) $data[$k1] = $v1;
            ...
            # 清空指定KEY
            elseif('set' == $opt){
                foreach ($fields[$table] as $k1 => $v1) $data[$k1] = '';
            }
            return $data;
        }
        return $fields[$table];
